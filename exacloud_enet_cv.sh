...
for DATA in o__ic50 ; do
        for HIDCOMP in 0; do

                FNAME=sub_enet_"$DATA"_"$HIDCOMP"_"$EXPFOLD"
                echo 'Executable = /usr/bin/Rscript' > $FNAME
                echo 'Universe = vanilla' >> $FNAME
                echo "Error = stderr/enet_$DATA.$HIDCOMP.err."'$(Cluster).$(Process)' >> $FNAME
                echo "Output = stdout/enet_$DATA.$HIDCOMP.out."'$(Cluster).$(Process)' >> $FNAME
                echo "K=$HIDCOMP" >> $FNAME
                echo 'RREST=0' >> $FNAME
                echo "SET=$DATAPANEL" >> $FNAME
                echo "EXP=$EXPFOLD" >> $FNAME
                echo "request_memory = $REQMEM GB" >> $FNAME
                echo "Arguments = --vanilla code/run.R cv enet  $DATA"  ' $(K) $(RREST) $(SET) $(EXP) results/enet_cv/' >> $FNAME
                echo 'queue' >> $FNAME
